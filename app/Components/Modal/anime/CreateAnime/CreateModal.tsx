...
const CreateModal = (props : PageProps) => {
  ...
  const form = useForm({
    validate: zodResolver(CreateAnimeDto),
    initialValues: {
     title : "" ,
     secondtitle : "" ,
     type : "" ,
  ...
  useEffect( () => {
    axios.get('/api/Gener/Get').then((res) => {
      SetGener(res.data)
    })
...
const { mutate } = useMutation(
  async (values ) => {
    return  axios.post('/api/anime/Create',values)
  },
  {
    onSuccess: (data) => {
  ...
  return (
    <div>
      ...
      <Text color='#fff' align='center' size={'1.3rem'}>انشاء انمي</Text>
    ...
    <Input.Wrapper label="Gener" style={{marginBottom:"1rem"}} styles={{'label' :{color:"#fff" , fontSize:"1.1rem"}}}>
    <Select
   w={200} 
      placeholder="اختر واحد"
      styles={{'input':{border:"0",background:"#ffff",color:"#000"}}} 
      data={gener}
      {...form.getInputProps('generId')}
    />
          </Input.Wrapper>       
    <Input.Wrapper label="Tags" style={{marginBottom:"1rem"}} styles={{'label' :{color:"#fff" , fontSize:"1.1rem"}}}>
    <MultiSelect
       data={tags}
       w={200} 
       placeholder="Pick all that you like"
       style={{marginBottom:"1rem"}} styles={{'label' :{color:"#fff" , fontSize:"1.1rem"},'wrapper':{background:"#fff"}}}
       {...form.getInputProps('tagsId')}
    />
          </Input.Wrapper>       
  ...
  <Text style={{color:"#fff"}} >{imageUrl}</Text>
     </Flex>
        </form>
