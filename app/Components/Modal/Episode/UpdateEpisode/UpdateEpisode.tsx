...
export const  UpdateEpisode = (props : PageProps) => {
  const form = useForm({
    initialValues: {
     id : props.id ,
      animeId : props.animeId,
      Servers:props.Servers,
      EpNumber : props.EpNumber ,
      EpName :props.EpName ,
      islast : props.islast ,
    ...
    validate : {
      Servers : {
        ServerName : (value) => (value.length ===0 ? "ServerName is Required" : null),
...
const { mutate } = useMutation(
 async (values ) => {
   return  axios.put('/api/Episodes/Update',values)
 },
 {
   onSuccess: (data) => {
  ...
  const fields = form.values.Servers.map((item, index) => (
    <Group  mt="xs">
      <TextInput
        ...
        sx={{ flex: 1 }}
        {...form.getInputProps(`Servers.${index}.ServerName`)}
        label="اضافة سيرفر"
  ...
  return (
    ...
    <TextInput  w={200}  type='text' styles={{'input':{border:"0",background:"#ffff",color:"#000"}}}  {...form.getInputProps('EpName')} />
      ...
      <Group position="center" mt="md" mb={30}>
        <Button
          onClick={() =>
            form.insertListItem('Servers', {  ServerName: '', ServerUrl: "" , id : 0 })
          }
        >
          اضافة سيرفر
        </Button>
